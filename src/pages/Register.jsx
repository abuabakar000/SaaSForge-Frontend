    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (accessToken && userData) {
                ...
                showNotification('Neural identity node initialized successfully.');
                navigate('/verify-email/pending', { state: { email }, replace: true });
            } else {
                navigate('/login');
            }
        } catch (err) {
            if (!err?.response) {
                setError('Neural synchronization failed. No server response.');
            } else if (err.response?.status === 409) {
                setError('Conflict detected: Identity or Email already registered.');
            } else {
                setError('Registry initialization failed. Please retry.');
            }
        } finally {
            setLoading(false);
        }
            ...
            <div className="w-full max-w-[480px] relative z-10">
                ...
                <div className="text-center mb-10 space-y-4">
                    ...
                    <h1 className="text-4xl font-black text-white tracking-tighter italic uppercase">
                        Initialize Registry<span className="text-indigo-500">.</span>
                    </h1>
                    <p className="text-slate-500 font-medium">Create your neural identity node to start building.</p>
                </div>
                ...
                <div className="bg-slate-900/40 backdrop-blur-xl p-10 rounded-[2.5rem] border border-white/10 premium-shadow">
                    ...
                    <form onSubmit={handleSubmit} className="space-y-6">
                        <div className="space-y-2">
                            <label className="text-[10px] font-black uppercase text-slate-500 tracking-[0.2em] ml-1">Identity Handle</label>
                        ...
                        <div className="space-y-2">
                            <label className="text-[10px] font-black uppercase text-slate-500 tracking-[0.2em] ml-1">Communication Channel</label>
                            <div className="relative">
                        ...
                        >
                            {loading ? (
                                ...
                                <>Initialize Registry <ArrowRight className="w-4 h-4" /></>
                            )}
                        </button>
                    </form>
                </div>
                ...
                <div className="mt-10 text-center">
                    <p className="text-slate-500 font-medium">
                        Already authenticated? <Link to="/login" className="text-white hover:text-indigo-400 font-black uppercase tracking-widest text-[10px] ml-2 transition-colors">Authorize Access</Link>
                    </p>
                </div>
            </div>
        </section>
